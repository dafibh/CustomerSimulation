...
class Product_Delete(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(topFrame, text="Delete Product", font=controller.title_font)
    ...
    def OnDoubleClick(self, event):
        ...
        MsgBox = tk.messagebox.askquestion ('Delete Record',f'Are you sure you want to delete record with ID : {id}?',icon = 'warning')
        if MsgBox == 'yes':
            for i in range(len(self.controller.productList)):
                if self.controller.productList[i].getID() == id:
                    del self.controller.productList[i]
                    self.controller.refreshTables()
                    break
            tk.messagebox.showinfo('Success','Deletion successful')
